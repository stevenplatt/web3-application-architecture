...
import { addMissingProofs, addMissingSignatures, ZkappCommand, AccountUpdate, ZkappPublicInput, TokenId, CallForest, Authorization, Actions, dummySignature, } from './account_update.js';
...
import { transactionCommitments, verifyAccountUpdateSignature, } from '../mina-signer/src/sign-zkapp-command.js';
...
function LocalBlockchain({ accountCreationFee = defaultAccountCreationFee, proofsEnabled = true, enforceTransactionLimits = true, } = {}) {
    ...
    return {
        ...
        async sendTransaction(txn) {
            ...
            let commitments = transactionCommitments(TypesBigint.ZkappCommand.fromJSON(zkappCommandJson));
...
let activeInstance = {
    ...
    hasAccount(publicKey, tokenId = TokenId.default) {
        if (!currentTransaction.has() ||
            currentTransaction.get().fetchMode === 'cached') {
            return !!Fetch.getCachedAccount(publicKey, tokenId, Fetch.networkConfig.minaEndpoint);
        }
        return false;
    },
    getAccount(publicKey, tokenId = TokenId.default) {
        if (currentTransaction()?.fetchMode === 'test') {
            Fetch.markAccountToBeFetched(publicKey, tokenId, Fetch.networkConfig.minaEndpoint);
            return dummyAccount(publicKey);
        }
        if (!currentTransaction.has() ||
            currentTransaction.get().fetchMode === 'cached') {
            let account = Fetch.getCachedAccount(publicKey, tokenId, Fetch.networkConfig.minaEndpoint);
            if (account === undefined)
...
async function verifyAccountUpdate(account, accountUpdate, transactionCommitments, proofsEnabled) {
    ...
    if (accountUpdate.authorization.signature) {
        ...
        try {
            isValidSignature = verifyAccountUpdateSignature(TypesBigint.AccountUpdate.fromJSON(accountUpdateJson), transactionCommitments, 'testnet');
        }
        catch (error) {
            errorTrace += '\n\n' + error.message;
...
function verifyTransactionLimits({ accountUpdates }) {
    ...
    */
    let totalTimeRequired = TransactionCost.PROOF_COST * authTypes.proof +
