...
import { Test, Pickles } from '../../snarky.js';
import { PrivateKey as PrivateKeySnarky, PublicKey as PublicKeySnarky, } from '../../lib/signature.js';
import { AccountUpdate as AccountUpdateSnarky, ZkappCommand as ZkappCommandSnarky, } from '../../lib/account_update.js';
import { PrivateKey, PublicKey } from '../../provable/curve-bigint.js';
import { AccountUpdate, Field, ZkappCommand, } from '../../bindings/mina-transaction/gen/transaction-bigint.js';
import * as TypesSnarky from '../../bindings/mina-transaction/gen/transaction.js';
import { accountUpdateFromFeePayer, accountUpdateHash, accountUpdatesToCallForest, callForestHash, feePayerHash, isCallDepthValid, signZkappCommand, verifyZkappCommandSignature, } from './sign-zkapp-command.js';
...
let dummyInputSnarky = MlHashInput.from(Test.hashInputFromJson.body(JSON.stringify(AccountUpdateSnarky.toJSON(dummySnarky).body)));
...
test(Random.accountUpdate, (accountUpdate) => {
    ...
    let accountUpdateJson = AccountUpdate.toJSON(accountUpdate);
    // account update hash
...
test(RandomTransaction.zkappCommand, (zkappCommand, assert) => {
    ...
    let ocamlCommitments = Test.hashFromJson.transactionCommitments(JSON.stringify(zkappCommandJson));
...
test.negative(RandomTransaction.zkappCommandJson.invalid, Random.json.privateKey, RandomTransaction.networkId, (zkappCommand, feePayerKey, networkId) => {
    signZkappCommand(zkappCommand, feePayerKey, networkId);
});
// zkapp transaction
test(RandomTransaction.zkappCommandAndFeePayerKey, ({ feePayerKey, zkappCommand }) => {
    zkappCommand.accountUpdates.forEach(fixVerificationKey);
    ...
    let feePayerInput1 = MlHashInput.from(Test.hashInputFromJson.body(JSON.stringify(feePayerJson.body)));
    ...
    expect(feePayerDigest).toEqual(FieldConst.toBigint(ocamlCommitments.feePayerHash));
    let fullCommitment = hashWithPrefix(prefixes.accountUpdateCons, [
        memoHash,
        feePayerDigest,
        commitment,
    ]);
    expect(fullCommitment).toEqual(FieldConst.toBigint(ocamlCommitments.fullCommitment));
    // signature
    ...
    let sigTestnetOcaml = Test.signature.signFieldElement(ocamlCommitments.fullCommitment, Ml.fromPrivateKey(feePayerKeySnarky), false);
    let sigMainnetOcaml = Test.signature.signFieldElement(ocamlCommitments.fullCommitment, Ml.fromPrivateKey(feePayerKeySnarky), true);
    expect(Signature.toBase58(sigTestnet)).toEqual(sigTestnetOcaml);
    expect(Signature.toBase58(sigMainnet)).toEqual(sigMainnetOcaml);
    let verify = (s, id) => verifyFieldElement(s, fullCommitment, feePayerAddress, id);
    ...
    expect(verifyZkappCommandSignature(sTest, feePayerAddressBase58, 'testnet')).toEqual(true);
    expect(verifyZkappCommandSignature(sTest, feePayerAddressBase58, 'mainnet')).toEqual(false);
    expect(verifyZkappCommandSignature(sMain, feePayerAddressBase58, 'testnet')).toEqual(false);
    expect(verifyZkappCommandSignature(sMain, feePayerAddressBase58, 'mainnet')).toEqual(true);
});
console.log('to/from json, hashes & signatures are consistent! 🎉');
function fixVerificationKey(a) {
...
function fixVerificationKeyHash(a) {
