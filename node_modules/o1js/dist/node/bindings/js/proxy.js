...
function proxyFunctions(getModuleObject, isItReady, functionNames) {
    ...
    for (let functionName of functionNames) {
        moduleProxy[functionName] = function (...args) {
            if (!isItReady())
                throw Error(functionError(functionName));
            return getModuleObject()[functionName](...args);
        };
    }
    return moduleProxy;
}
let constructError = (className) => `Cannot call class constructor because o1js has not finished loading.
Try calling \`await isReady\` before \`new ${className}()\``;
let methodError = (className, methodName) => `Cannot call static method because o1js has not finished loading.
Try calling \`await isReady\` before \`${className}.${methodName}()\``;
let functionError = (functionName) => `Cannot call function because o1js has not finished loading.
