var _SmartContract_executionState, _SmartContract__senderState;
import { __classPrivateFieldGet, __classPrivateFieldSet } from "tslib";
import { Pickles } from '../snarky.js';
import { Field, Bool } from './core.js';
import { AccountUpdate, Authorization, Body, Events, Permissions, Actions, smartContractContext, TokenId, ZkappCommand, zkAppProver, ZkappPublicInput, ZkappStateLength, } from './account_update.js';
import { cloneCircuitValue, provable, Struct, toConstant, } from './circuit_value.js';
...
import { analyzeMethod, compileProgram, Empty, emptyValue, GenericArgument, getPreviousProofsForProver, isAsFields, methodArgumentsToConstant, methodArgumentTypesAndValues, Proof, sortMethodArguments, } from './proof_system.js';
import { PublicKey } from './signature.js';
import { assertStatePrecondition, cleanStatePrecondition } from './state.js';
import { inAnalyze, inCompile, inProver, snarkContext, } from './provable-context.js';
// external API
export { SmartContract, method, declareMethods, Callback, Account, VerificationKey, Reducer, };
...
function method(target, methodName, descriptor) {
    ...
    SelfProof.publicOutputType = Empty;
    SelfProof.tag = () => ZkappClass;
    let internalMethodEntry = sortMethodArguments(ZkappClass.name, methodName, paramTypes, SelfProof);
    // add witness arguments for the publicKey (address) and tokenId
    let methodEntry = sortMethodArguments(ZkappClass.name, methodName, [PublicKey, Field, ...paramTypes], SelfProof);
...
function wrapMethod(method, ZkappClass, methodIntf) {
    ...
    return function wrappedMethod(...actualArgs) {
        ...
        if (!insideContract) {
            ...
            try {
                if (inCompile() || inProver() || inAnalyze()) {
                    ...
                    try {
                        ...
                        if (DEBUG_PUBLIC_INPUT_CHECK) {
                            Provable.asProver(() => {
                                ...
                                let { accountUpdate: inputUpdate, transaction, index, } = zkAppProver.getData();
                    ...
                    let result;
                    try {
                        result = method.apply(this, actualArgs.map((a, i) => {
        ...
        try {
            ...
            if (ZkappClass._methodMetadata?.[methodIntf.methodName]?.hasReturn &&
            ...
            let { accountUpdate, result } = methodCallDepth === 0
                ? AccountUpdate.witness(returnType ?? provable(null), runCalledContract, { skipCheck: true })
                : runCalledContract();
...
class Callback extends GenericArgument {
    static create(instance, methodName, args) {
        ...
        let methodIntf_ = (ZkappClass._methods ?? []).find((i) => i.methodName === methodName);
...
class SmartContract {
    ...
    static async compile() {
        ...
        this.analyzeMethods();
        let { verificationKey: verificationKey_, provers, verify, } = await compileProgram(ZkappPublicInput, Empty, methodIntfs, methods, this);
        let verificationKey = {
    ...
    deploy({ verificationKey, zkappKey, } = {}) {
        ...
        verificationKey ?? (verificationKey = this.constructor
            ._verificationKey);
        ...
        let shouldInit = !Mina.hasAccount(this.address) ||
        ...
        __classPrivateFieldSet(this, _SmartContract_executionState, {
            transactionId: __classPrivateFieldGet(this, _SmartContract_executionState, "f").transactionId,
            accountUpdate,
        }, "f");
...
`);
    ...
    get self() {
        ...
        if (inSmartContract && inSmartContract.this === this) {
            ...
            __classPrivateFieldSet(this, _SmartContract_executionState, { accountUpdate, transactionId }, "f");
            return accountUpdate;
        }
        let executionState = __classPrivateFieldGet(this, _SmartContract_executionState, "f");
        ...
        __classPrivateFieldSet(this, _SmartContract_executionState, { transactionId, accountUpdate }, "f");
        return accountUpdate;
    }
    ...
    get sender() {
        ...
        if (__classPrivateFieldGet(this, _SmartContract__senderState, "f")?.transactionId === transactionId) {
            return __classPrivateFieldGet(this, _SmartContract__senderState, "f").sender;
        }
        else {
            let sender = Provable.witness(PublicKey, () => Mina.sender());
            __classPrivateFieldSet(this, _SmartContract__senderState, { transactionId, sender }, "f");
            return sender;
        }
    }
    /**
    ...
    static analyzeMethods() {
        ...
        if (!methodIntfs.every((m) => m.methodName in methodMetadata) &&
            !inAnalyze()) {
            ...
            try {
                for (let methodIntf of methodIntfs) {
                    ...
                    let { rows, digest, result, gates } = analyzeMethod(ZkappPublicInput, methodIntf, (publicInput, publicKey, tokenId, ...args) => {
                        ...
                        let result = instance[methodIntf.methodName](publicInput, ...args);
                        accountUpdate = __classPrivateFieldGet(instance, _SmartContract_executionState, "f").accountUpdate;
                        return result;
                    });
...
_SmartContract_executionState = new WeakMap(), _SmartContract__senderState = new WeakMap();
...
class ${contract.constructor.name} extends SmartContract {
  reducer = { actionType: Field };
    ...
    return {
        ...
        reduce(actionLists, stateType, reduce, { state, actionState }, { maxTransactionsWithActions = 32, skipActionStatePrecondition = false, } = {}) {
...
Use the optional \`maxTransactionsWithActions\` argument to increase this number.`);
            ...
            for (let i = 0; i < maxTransactionsWithActions; i++) {
                ...
                let lengths = possibleActionsPerTransaction.map((n) => Provable.witness(Bool, () => Bool(length === n)));
        ...
        forEach(actionLists, callback, fromActionState, config) {
            ...
            let { actionState } = this.reduce(actionLists, stateType, (_, action) => {
                callback(action);
                return undefined;
            }, { state: undefined, actionState: fromActionState }, config);
            return actionState;
        },
        getActions(config) {
            let actionsForAccount = [];
            Provable.asProver(() => {
                let actions = Mina.getActions(contract.address, config, contract.self.tokenId);
        ...
        async fetchActions(config) {
            let result = await Mina.fetchActions(contract.address, config, contract.self.tokenId);
            if ('error' in result) {
