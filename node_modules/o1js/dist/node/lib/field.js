var _Field_instances, _a, _Field_isField, _Field_toConst, _Field_toVar, _Field_toConstant, _Field_compare, _Field_checkBitLength;
...
export { Field };
// internal API
export { FieldType, FieldVar, FieldConst, isField, withMessage, readVarMessage, toConstantField, };
...
class Field {
    ...
    constructor(x) {
        ...
        if (__classPrivateFieldGet(Field, _a, "m", _Field_isField).call(Field, x)) {
            this.value = x.value;
            return;
        }
        // FieldVar
     ...
     */
    toConstant() {
        return __classPrivateFieldGet(this, _Field_instances, "m", _Field_toConstant).call(this, 'toConstant');
    }
    /**
     ...
     */
    toBigInt() {
        let x = __classPrivateFieldGet(this, _Field_instances, "m", _Field_toConstant).call(this, 'toBigInt');
     ...
     */
    toString() {
        return __classPrivateFieldGet(this, _Field_instances, "m", _Field_toConstant).call(this, 'toString').toBigInt().toString();
    }
    /**
    ...
    assertEquals(y, message) {
        try {
            ...
            Snarky.field.assertEqual(this.value, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y));
        }
        catch (err) {
    ...
    add(y) {
        ...
        let z = Snarky.field.add(this.value, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y));
        return new Field(z);
    }
    /**
    ...
    isEven() {
        if (this.isConstant())
            return new Bool(this.toBigInt() % 2n === 0n);
    ...
    mul(y) {
        ...
        if (isConstant(y)) {
            let z = Snarky.field.scale(__classPrivateFieldGet(Field, _a, "m", _Field_toConst).call(Field, y), this.value);
            return new Field(z);
        }
        if (this.isConstant()) {
        ...
        let z = Snarky.existsVar(() => FieldConst.fromBigint(Fp.mul(this.toBigInt(), toFp(y))));
    ...
    square() {
        ...
        let z = Snarky.existsVar(() => FieldConst.fromBigint(Fp.square(this.toBigInt())));
    ...
    isZero() {
        ...
        // z * x === 1 - b
        Snarky.field.assertMul(z, this.value, Snarky.field.add(FieldVar[1], Snarky.field.scale(FieldConst[-1], b)));
    ...
    lessThan(y) {
        ...
        return __classPrivateFieldGet(this, _Field_instances, "m", _Field_compare).call(this, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y)).less;
    }
    /**
    ...
    lessThanOrEqual(y) {
        ...
        return __classPrivateFieldGet(this, _Field_instances, "m", _Field_compare).call(this, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y)).lessOrEqual;
    }
    /**
    ...
    assertLessThan(y, message) {
        try {
            ...
            let { less } = __classPrivateFieldGet(this, _Field_instances, "m", _Field_compare).call(this, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y));
            less.assertTrue();
        }
        catch (err) {
            throw withMessage(err, message);
    ...
    assertLessThanOrEqual(y, message) {
        try {
            ...
            let { lessOrEqual } = __classPrivateFieldGet(this, _Field_instances, "m", _Field_compare).call(this, __classPrivateFieldGet(Field, _a, "m", _Field_toVar).call(Field, y));
    ...
    toBits(length) {
        if (length !== undefined)
            __classPrivateFieldGet(Field, _a, "m", _Field_checkBitLength).call(Field, 'Field.toBits()', length);
        if (this.isConstant()) {
            let bits = Fp.toBits(this.toBigInt());
            if (length !== undefined) {
    ...
    static fromBits(bits) {
        ...
        __classPrivateFieldGet(Field, _a, "m", _Field_checkBitLength).call(Field, 'Field.fromBits()', length);
        ...
        let bitsVars = bits.map((b) => {
            if (typeof b === 'boolean')
                return b ? FieldVar[1] : FieldVar[0];
    ...
    rangeCheckHelper(length) {
        __classPrivateFieldGet(Field, _a, "m", _Field_checkBitLength).call(Field, 'Field.rangeCheckHelper()', length);
        if (length % 16 !== 0)
     ...
     */
    toJSON() {
        return __classPrivateFieldGet(this, _Field_instances, "m", _Field_toConstant).call(this, 'toJSON').toString();
    }
    /**
...
_a = Field, _Field_instances = new WeakSet(), _Field_isField = function _Field_isField(x) {
    return x instanceof Field;
}, _Field_toConst = function _Field_toConst(x) {
    if (__classPrivateFieldGet(Field, _a, "m", _Field_isField).call(Field, x))
        return x.value[1];
    return FieldConst.fromBigint(Fp(x));
}, _Field_toVar = function _Field_toVar(x) {
    if (__classPrivateFieldGet(Field, _a, "m", _Field_isField).call(Field, x))
        return x.value;
    return FieldVar.constant(Fp(x));
}, _Field_toConstant = function _Field_toConstant(name) {
    return toConstantField(this, name, 'x', 'field element');
}, _Field_compare = function _Field_compare(y) {
    // TODO: support all bit lengths
    let maxLength = Fp.sizeInBits - 2;
    asProver(() => {
        let actualLength = Math.max(this.toBigInt().toString(2).length, new Field(y).toBigInt().toString(2).length);
...
}, _Field_checkBitLength = function _Field_checkBitLength(name, length) {
    if (length > Fp.sizeInBits)
        throw Error(`${name}: bit length must be ${Fp.sizeInBits} or less, got ${length}`);
    if (length <= 0)
