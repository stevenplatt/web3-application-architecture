...
declare const test: (<T extends readonly Random<any>[]>(...args: [...gens: T, run: (...args: [...values: Nexts<T> extends infer T_1 extends readonly (() => any)[] ? { [i in keyof T_1]: Nexts<T>[i] extends () => infer U ? U : never; } : never, assert: (b: boolean, message?: string) => void]) => void]) => number) & {
    negative: <T extends readonly Random<any>[]>(...args: [...gens: T, run: (...args: [...values: Nexts<T> extends infer T_2 extends readonly (() => any)[] ? { [i in keyof T_2]: Nexts<T>[i] extends () => infer U ? U : never; } : never, assert: (b: boolean, message?: string) => void]) => void]) => number;
...
}): <T extends readonly Random<any>[]>(...args: [...gens: T, run: (...args: [...values: Nexts<T> extends infer T_1 extends readonly (() => any)[] ? { [i in keyof T_1]: Nexts<T>[i] extends () => infer U ? U : never; } : never, assert: (b: boolean, message?: string | undefined) => void]) => void]) => number;
